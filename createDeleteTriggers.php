<?php
/**
 * Script to create delete triggers for database tables
 * 
 ...
 * 
 * Usage:
 *   php createDeleteTriggers.php                    # Create triggers for all tables
 *   php createDeleteTriggers.php -t table_name      # Create trigger for specific table only
 */

class Pman_Core_createDeleteTriggers {
...
if (php_sapi_name() === 'cli') {
    $opts = getopt('t:', array('table:'));
    $processed_opts = array();
    
    if (isset($opts['t'])) {
        $processed_opts['table'] = $opts['t'];
    } elseif (isset($opts['table'])) {
        $processed_opts['table'] = $opts['table'];
    }
    
    new Pman_Core_createDeleteTriggers($processed_opts);
}

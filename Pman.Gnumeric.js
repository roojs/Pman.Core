 ...
 * x = new Pman.Gnumeric( {
   ...
   x.data({ ... }); // key value data looks for {value} in strings and replaces it..
   x.set('A3', 'test');
...
cset : function(cell, v) {
            ...
            parseDoc : function() 
            {
                ...
                function rangeToRC(s) {
                    var ar = s.split(':');
                    return [ toRC(ar[0]) , toRC(ar[1])]
                }
                var defaultCell = {
                            valueType : 0,
                            valueFormat : '',
                ...
                var sheet = _this.dialog.xml.getElementsByTagName('Sheet')[0];
                ...
                for (var r = 0; r < rmax;r++) {
                 ...
                 Roo.each(merge, function(c) {
                    var rc = rangeToRC(c.textContent);

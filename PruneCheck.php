    ...
    function outputNagiosResults()
    {
        ...
        foreach ($this->results as $table => $result) {
            ...
            $status_messages[] = sprintf(
                ...
                " (%d " . (isset($result['prunable_groups_per_run']) ? 'groups of record' : 'records') . " per run)",
                $table,
                $result['prunable_records'],
                $result['total_records'],
                $result['runs_needed'],
                isset($result['prunable_groups_per_run']) ? $result['prunable_groups_per_run'] : $result['prunable_records_per_run']
            );

...
class Pman_Core_NotifySend extends Pman
{
    ...
    {   
       ...
       // }
        ...
        if(!$core_domain->has_mx && (empty($core_domain->mx_updated) || strtotime($core_domain->mx_updated) > strtotime('now - 5 day'))) {
                ...
                foreach ($ff->Core_Notify['routes'] as $server => $settings){
                    ...
                    if(!empty($settings['auth']) && $settings['auth'] == 'XOAUTH2') {
                        ...
                        if($fromUser->is_reply_to_only) {
                            ...
                            $addressStr = empty($fromUser->name) ? 
                                $fromUser->email:
                                "{$fromUser->name} <{$fromUser->email}>";
                            $email['headers']['From'] = $fromUser->email;
                            var_dump($email['headers']);
                            die('test');
                        }
